...
// monologue stays within app/pipeline/speaker_names.py's staleness cap.
...
function nameFromSpeakerTile(tile) {
  ...
  // Fallback only for a single-line, space-containing, non-badge string --
  // a layout that shows the name just once. Anything else (including the
  // multi-line presentation tile) yields no name.
  if (
    lines.length === 1 &&
    lines[0].includes(" ") &&
...
function findPeoplePanelToggleButton() {
  ...
  // noteTilePulse() above: must be re-verified against a live
